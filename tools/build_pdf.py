...
def implementation_coverage_table():
    """Покриття міської звітності про реалізацію проєктів."""
    rows = [
        ['Офіційний пооб’єктний реєстр або агрегований звіт', '22 / 35 (62,9 %)',
         'Підтверджує реалізацію на рівні проєктів або міського підсумку.'],
        ['Реєстр/агрегат або портал статусів', '27 / 35 (77,1 %)',
         'Портал статусів не є бухгалтерським підтвердженням видатків.'],
        ['Повний офіційний пооб’єктний реєстр', '9 / 35 (25,7 %)',
         'Придатний для частки завершення лише після звірки з переліком переможців.'],
    ]
    return make_table(
        ['Рівень доказу', 'Покриття', 'Межа інтерпретації'],
        rows, [6.3*cm, 3.0*cm, 8.0*cm], aligns=['l', 'c', 'l'])


def implementation_patterns_table():
    """Описові відмінності у доступності сильних джерел."""
    rows = [
        ['100–199 тис. осіб', '9', '4 (44,4 %)'],
        ['200–499 тис. осіб', '19', '13 (68,4 %)'],
        ['500 тис. осіб і більше', '7', '5 (71,4 %)'],
        ['Захід*', '8', '4 (50,0 %)'],
        ['Центр і Північ*', '11', '8 (72,7 %)'],
        ['Південь і Схід*', '16', '10 (62,5 %)'],
    ]
    return make_table(
        ['Група', 'Кількість міст', 'Офіційний реєстр або агрегат'],
        rows, [7.2*cm, 4.1*cm, 6.0*cm], aligns=['l', 'c', 'c'])


def size_resource_table():
    """Довоєнні показники ресурсної глибини за розміром міста."""
    rows = [
        ['100–199 тис.', '2017', '24 / 36', '0,2113 %', '0,429'],
        ['200–499 тис.', '2016', '60 / 76', '0,3288 %', '0,317'],
        ['500 тис. і більше', '2016', '27 / 28', '0,3038 %', '0,360'],
    ]
    return make_table(
        ['Населення', 'Медіанний рік старту', 'Покриття частки',
         'Зважена частка БУ', 'Заявок на 1000 осіб'],
        rows, [3.5*cm, 3.4*cm, 3.1*cm, 3.3*cm, 3.3*cm],
        aligns=['l', 'c', 'c', 'c', 'c'])


def macroregion_resource_table():
    """Довоєнні показники ресурсної глибини за аналітичними макрорегіонами."""
    rows = [
        ['Захід', '8 / 8', '29 / 32', '0,3554 %', '0,3800 %'],
        ['Центр і Північ', '7 / 11', '34 / 44', '0,3034 %', '0,2903 %'],
        ['Південь і Схід', '5 / 16', '48 / 64', '0,2348 %', '0,2955 %'],
    ]
    return make_table(
        ['Макрорегіон', 'Старт до кінця 2016', 'Покриття частки',
         'Медіана', 'Зважена частка'],
        rows, [3.7*cm, 3.7*cm, 3.3*cm, 3.0*cm, 3.0*cm],
        aligns=['l', 'c', 'c', 'c', 'c'])
...
P('<b>Походження і перенесення практики в Україну.</b> Сучасна модель бюджету участі сформувалася 1989 року '
  'в Порту-Алегрі (Бразилія) у контексті демократизації та децентралізації; її ядро — поєднання '
  'відкритого обговорення міських потреб із реальним впливом мешканців на пріоритети публічних інвестицій. '
  'В Україну практика прийшла через європейські, зокрема польські, адаптації та мережі міжнародної співпраці: '
  'у 2015 році PAUCI підтримала пілотне впровадження у Черкасах, Чернігові та Полтаві. У кількісній вибірці '
  'цієї статті за 2015 рік повні показники підтверджено для двох міст, тому це не тотожне числу всіх ранніх запусків.')
...
P('<b>Поширення: платформи та EGAP.</b> '
  ...
  'публічного голосування. До 2025 року механізм закріплювався переважно локально: кожне місто '
  'ухвалювало власне положення, а кошти передбачалися у видатках міського бюджету, а не '
  'в окремому позабюджетному фонді. Закон № 3703-IX, чинний із 8 січня 2025 року, прямо '
  'передбачив участь жителів у плануванні й розподілі коштів місцевого бюджету, але не '
  'встановив єдиної процедури чи обов’язкової відсоткової частки. Переломним став 2016 рік: '
  'за дванадцять місяців механізм '
...
P('Поширення механізму — лише одна сторона. Друга, важливіша для оцінки глибини участі, — '
  ...
  'Для довоєнного періоду 2018–2021 років медіана становить 0,2991 %, а зважена частка — '
  '0,3059 %. Це емпіричний центр української практики, а не нормативний мінімум. '
  'Значення 0,5 % і 1 % використовуються лише як порівняльні орієнтири; '
  'загальнодержавної відсоткової норми в Україні не існує.')
...
P('За середнім ховається значна міжміська нерівність. Лідери спрямовують на бюджет '
  ...
  'не пояснюється самою наявністю платформи: відмінності можуть відображати сталість '
  'процедури, бюджетну спроможність, локальні правила й політичний пріоритет механізму. '
  'Міста з коротким рядом, зокрема Черкаси з двома зіставними роками, не слід подавати '
  'як стабільних довгострокових лідерів.')

P('Усі 35 міст: розмір і суспільно-географічна диференціація', 'h2')
P('Абсолютний масштаб програми, інтенсивність подання заявок і частка бюджету участі '
  'у видатках міста є різними вимірами. Тому порівняння проведено для всієї вибірки, '
  'а не для одного великого й одного малого міста.', 'lead')
P('Таблиця 4. Довоєнні показники за групами чисельності населення', 'h3')
story.append(size_resource_table())
P('Міста з населенням 100–199 тис. осіб мали нижчу ресурсну частку, але не нижчу '
  'інтенсивність подання заявок. Кореляція населення із часткою бюджету участі слабка '
  '(Spearman ρ = 0,231), як і кореляція населення з роком запуску (ρ = -0,142). '
  'Розмір міста не можна трактувати як самостійне причинне пояснення.', 'cap')
P('Таблиця 5. Довоєнна макрорегіональна диференціація', 'h3')
story.append(macroregion_resource_table())
P('Макрорегіони сформовано аналітично, а не нормативно. Захід вирізняється ранньою '
  'дифузією та вищою типовою часткою; нижча медіана Півдня і Сходу не доводить нижчої '
  'громадської активності. У 2023–2026 роках 12 із 15 завершених відборів із визначеними '
  'переможцями припадають на західні міста; вони акумулюють 223,02 млн грн, або 93,4 % '
  'суми цих циклів. Це профіль документованого відновлення, а не доказ відсутності '
  'практик в інших регіонах.', 'cap')
P('Київ формує близько 35,0 % документованої абсолютної суми масиву, але його зважена '
  'частка становить лише 0,2649 %. Цей контраст показує, що абсолютний масштаб програми '
  'не тотожний її відносній ресурсній глибині.')
...
P('Універсального «правильного відсотка» не існує — UN-Habitat прямо зазначає '
  ...
  '≈ 1 % загального), окремий європейський оглядовий орієнтир (~1 %) та офіційна ціль '
  'Шотландії (щонайменше 1 %) лежать помітно вище. Це вибрані компаратори, а не '
  'універсальна норма. Коректність порівняння тримається '
...
P('Таблиця 6. Міжнародні орієнтири та знаменники', 'h3')
intl_rows = [
...
P('За 2015–2026 роки бюджет участі пройшов шлях від двох міст-піонерів до '
  ...
  'видатків. Це нижче за вибрані орієнтири 0,5 % і 1 %, але жоден із них не є '
  'обов’язковою нормою для українських міст.')
P('Це узгоджується з висновком суспільно-географічного аналізу партисипативних процесів: '
  ...
  'виконання. Вона має охоплювати не лише перелік переможців і планові суми, а й фінальний '
  'статус проєкту та, за можливості, фактичні видатки. Лише за такої умови участь '
  'перетворюється із символічного жесту на керований ресурс міського розвитку.')

# ---- реалізація та звітність ----
P('Реалізація проєктів: що дозволяє встановити звітність', 'h2')
P('Перемога в голосуванні та виділення коштів не є тотожними фактичному завершенню '
  'проєкту. Тому для оцінки результативності бюджетів участі окремо проаналізовано '
  'доступність звітів про реалізацію проєктів.', 'lead')
P('У вибірці 35 міст офіційний пооб’єктний реєстр або агрегований звіт про реалізацію '
  'знайдено для 22 міст (62,9 %); якщо додати муніципальні портали зі статусами проєктів — '
  'для 27 міст (77,1 %). Водночас повний пооб’єктний офіційний реєстр наявний лише для '
  '9 міст (25,7 %).')
story.append(implementation_coverage_table())
P('Консервативний аудит реалізаційної звітності охоплює 176 міських циклів 2015–2022 років. '
  'Зіставлення року циклу з роком, зазначеним у джерелі реалізації, дає '
  'офіційний реєстр або агрегований звіт для 74 циклів (42,0 %); з урахуванням порталів '
  'статусів — для 89 циклів (50,6 %). Це показник покриття джерелами, а не '
  'загальноукраїнська частка завершених проєктів, оскільки звіт про реалізацію може бути '
  'оприлюднений після року проведення конкурсу.')
P('У матриці діяльності окремо підтверджено 15 завершених відборів із визначеними '
  'переможцями у 2023–2026 роках. Їх ще не включено до знаменника 176 через лаг між '
  'відбором, реалізацією та публікацією фінального звіту. Найвище покриття в корпусі '
  'аудиту простежується для циклів 2018–2021 років, коли частка циклів зі '
...
P('Описові відмінності за розміром і розташуванням міст', 'h3')
story.append(implementation_patterns_table())
P('* Макрорегіони сформовано для аналітичного, а не офіційного, порівняння. На Заході '
  'всі 8 міст мають принаймні портал статусів, але лише 4 — офіційний реєстр або '
  'агрегований звіт. Виявлені відмінності характеризують доступність і тип документації; '
  'вони не доводять причинного зв’язку між розміром чи розташуванням міста та фактичною '
  'реалізацією проєктів.', 'cap')
P('Тестовий локальний випадок Білої Церкви показує, як має обчислюватися показник '
  ...
  'екстраполювати на всю Україну. Для загальнонаціонального показника потрібне поєднання '
  'переліку переможців, фінального статусу кожного проєкту та, бажано, даних про '
  'закупівлі або фактичні платежі.')

P('Висновки', 'h2')
P('Бюджет участі у великих містах України пройшов шлях від пілотів 2015 року до широкої, '
  'але неоднаково глибокої інституціоналізації: у 2018 році активні цикли підтверджено '
  'для 32 із 35 міст, а в останній повний довоєнний період типовий ресурсний рівень '
  'становив близько 0,3 % міських видатків. Швидка дифузія цифрових платформ не була '
  'тотожною ані суттєвому делегуванню бюджетного ресурсу, ані уніфікованій підзвітності.')
P('Розмір міста слабо пов’язаний із ресурсною часткою, тоді як рання дифузія, сталість '
  'процедур і форма публічної документації утворюють виразніші територіальні контрасти. '
  'Після 2022 року механізм просторово вибірково реконфігурувався: 12 із 15 документованих '
  'завершених відборів 2023–2026 років припадають на західні міста. Цей результат має '
  'описовий характер і не доводить причинного впливу регіону.')
P('Найсуттєвіша межа доказів стосується реалізації. Показники 74/176 і 89/176 '
  'характеризують покриття джерелами корпусу циклів 2015–2022 років, а не частку '
  'завершених проєктів. Для оцінки фактичного виконання потрібно поєднати перелік '
  'переможців, фінальний статус, касові видатки й, за можливості, закупівельне '
  'підтвердження. Саме завершений публічний цикл «подання — відбір — фінансування — '
  'виконання — звітування» перетворює бюджет участі з цифрової процедури на інструмент '
  'міського розвитку.')
...
P('Чисельник частки — сума бюджету участі — зібрано насамперед з офіційних матеріалів '
  ...
  'Знаменник — сума показника ZAT_AMT за всіма фондами (FUND_TYP=T; T=total, тобто '
  'загальний і спеціальний фонди разом) у найпізнішому доступному місячному звіті року '
  'з Open Budget API Мінфіну. Це затверджений план, а не касові видатки або уточнений план. '
  'Реєстр містить 470 записів '
...
chronicle = [
    ('2015', 'У вибірці — два міста з повними кількісними показниками: 127 заявок, 23 переможці, '
    ...
    ('2021', 'Останній повний довоєнний рік: рекордні ~648 млн грн і 1,2 тис. переможців. Але в '
             'частці видатків це ~0,3 %. Значення 0,5 % є порівняльним орієнтиром, а не '
             'обов’язковим мінімумом. Поширеність ≠ глибина.'),
...
sources = [
    ...
    'Авторський реєстр покриття звітністю про реалізацію проєктів бюджетів участі, 2015–2026 '
    '(35 міських папок, збережені первинні та муніципальні документи, класи доказу і межі використання).',
    'Santos B. de Sousa. Participatory Budgeting in Porto Alegre: Toward a Redistributive Democracy. '
    'Politics & Society. 1998. Vol. 26(4). DOI: 10.1177/0032329298026004003.',
    'Sintomer Y., Herzberg C., Röcke A. Participatory Budgeting in Europe: Potentials and Challenges. '
    'International Journal of Urban and Regional Research. 2008. Vol. 32(1). DOI: 10.1111/j.1468-2427.2008.00777.x.',
    'Khutkyy D., Avramchenko K. Impact Evaluation of Participatory Budgeting in Ukraine. 2019. '
    'URL: https://www.oidp.net/docs/repo/doc671.pdf.',
    'Schmäing S. Urban Democracy in Post-Maidan Ukraine. European Societies. 2024. Vol. 26(2). '
    'DOI: 10.1080/14616696.2023.2183972.',
    'Schmäing S. From Reconfiguring the Urban Space to Resisting Russia’s War: '
    'Local Civic Engagement in Ukraine since 2014. Post-Soviet Affairs. 2025. '
    'Vol. 41(5). P. 478–497. DOI: 10.1080/1060586X.2025.2491969.',
    'Kuruppu C., Maksymchuk O., Adhikari P. Exploring Elitisation of Participatory Budgeting '
    'in a Post-Soviet Democracy. Journal of Accounting in Emerging Economies. 2023. '
    'DOI: 10.1108/JAEE-10-2018-0118.',
    'PAUCI. Annual Report 2016: методична підтримка українських міських пілотів.',
    'Swiss-Ukrainian EGAP Program. How Participatory Budgeting Influences Local Decision-Making in Ukraine.',
    'OECD. Public Governance Review of Ukraine. 2026. DOI: 10.1787/7810f059-en.',
    'Закон України від 09.05.2024 № 3703-IX. URL: https://zakon.rada.gov.ua/laws/show/3703-IX.',
    'Наказ Міністерства фінансів України від 03.03.2020 № 94. '
    'URL: https://zakon.rada.gov.ua/go/v0094201-20.',
...
# ==================================================================== ЗБІРКА
OUT = os.environ.get('PB_PDF_OUT', os.path.join(PROJECT, 'Стаття_Бюджети_участі_2015-2026.pdf'))
...
def repair_map_headings(path):
    """Усуває накладання векторних карт на заголовки сторінок додатка A."""
    import fitz

    document = fitz.open(path)
    changed = False
    map_headings = [
        ('2021. Пік', 'Карта A.3. Бюджети участі, 2021 рік'),
        ('Розмір бульбашки — сумарний обсяг', 'Карта A.5. Сумарний бюджет участі за 2015–2026 роки'),
    ]
    for page in document:
        text = page.get_text()
        for marker, heading in map_headings:
            if marker not in text:
                continue
            page.draw_rect(fitz.Rect(0, 0, page.rect.width, 90), color=None, fill=(1, 1, 1), overlay=True)
            page.insert_text(
                (2 * cm, 41), heading,
