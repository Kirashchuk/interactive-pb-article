...
P('<b>Поширення: платформи та EGAP.</b> '
  'У 2015 році перші конкурси оголосили Чернігів і Черкаси. Далі поширення пішло через '
...
for i, (yr, tag, cap) in enumerate(map_years):
    if i > 0:
        story.append(PageBreak())
    P(f'Карта {tag}. Бюджети участі, {yr} рік', 'h3')
    story.append(mm.draw(yr))
    story.append(pdf_map.legend(width=460))
    P(f'<b>{yr}.</b> {cap}. Розмір бульбашки пропорційний сумі бюджету участі міста за рік; '
      'заливка області — сума по її містах вибірки.', 'cap')

# Сумарна карта за весь період (агрегація по містах)
story.append(PageBreak())
P('Карта A.5. Сумарний бюджет участі за 2015–2026 роки', 'h3')
agg_cities = []
for c in pb['cities']:
...
story.append(mm_all.draw('ALL'))
story.append(pdf_map.legend(width=460))
P('Розмір бульбашки — сумарний обсяг бюджету участі міста за 2015–2026 роки; заливка '
  'області — сума по її містах вибірки. Найбільші програми — Київ, Одеса, Львів, Дніпро, '
  'Івано-Франківськ, Харків.', 'cap')
