...
# ---------------------------------------------------------- Аналітичний_поріг
ws = wb['Аналітичний_поріг']
thr_rows = [list(r) for r in ws.iter_rows(values_only=True)]
thresholds = []
for r in thr_rows:
    if r and fnum(r[1]) is not None and r[0] and str(r[0]).strip() and fnum(r[1]) <= 0.05:
        label = str(r[0]).strip()
        if label.startswith(('Стартовий', 'Інституційний', 'Зрілий', '5%')):
            thresholds.append({'label': label, 'value': fnum(r[1]),
